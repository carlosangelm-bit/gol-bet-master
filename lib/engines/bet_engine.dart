...
class BetEngine {
  ...
  static double? canonicalSlidingBetween(Round round, String p1Id, String p2Id) =>
      round.acuerdoDeVentaja(p1Id, p2Id);

  // ══════════════════════════════════════════════════════════════════════════
  // VALIDACIÓN DE pairSliding
  // ══════════════════════════════════════════════════════════════════════════

  /// Valida la coherencia de `pairSliding` y sus conflictos con el legacy.
  ///
  /// Se mudó a [Round.erroresDeVentaja]: es una validación de los DATOS de la
  /// ronda, y tenerla aquí obligaba al motor a leer los dos almacenes a mano —
  /// lo que la guarda de «una sola fuente» detecta, con razón.
  static List<String> validatePairSliding(Round round) =>
      round.erroresDeVentaja();

  // ══════════════════════════════════════════════════════════════════════════
  // MIGRACIÓN LEGACY: manualHandicaps → pairSliding
  // ══════════════════════════════════════════════════════════════════════════

  // La migración del formato viejo —`buildPairSlidingFromLegacy`— se mudó a
  // `round_provider`, con las demás migraciones. Aquí no puede quedar ni un
  // alias: el provider importa el motor, así que llamarlo desde aquí sería un
  // ciclo. Y no hacía falta: en producción solo la usaba el provider.
  ...
  /// Cuántos golpes recibe p1 de p2. **Una línea a propósito.**
  ///
  /// Aquí vivía la cadena entera —pairSliding, el legacy bilateral y la resta de
  /// handicaps— y era una de NUEVE copias. Ahora vive en [Round.ventajaDe], que
  /// es donde están los datos y a quien puede llamar todo el mundo: el modelo no
  /// importa los motores, y esa era justo la razón de que cada pantalla
  /// escribiera la suya.
  ///
  /// `estricto` se queda: la liquidación prefiere fallar a cobrar mal cuando el
  /// acuerdo legacy es contradictorio.
  static double _strokesP1ReceivesFromP2(
          Round round, String p1Id, String p2Id) =>
      round.ventajaDe(p1Id, p2Id, estricto: true);

  /// Método público: cuántos golpes recibe p1 de p2.
  ...
  static bool hasExplicitAgreement(Round round, String p1Id, String p2Id) =>
      round.hayAcuerdoDeVentaja(p1Id, p2Id);
  ...
  static ValoresDelNassau valoresDelNassau(
    ...
    //
    // Y con los dos nueves a cero no hay nada que trasladar en ningún caso: el
    // F9 no vale nada. Se descarta aquí y no solo en la pantalla, para que el
  ...
  static List<LineaDelDuelo> lineasDelDuelo(
    ...
    LineaDelDuelo? linea(
      ...
      return LineaDelDuelo(
        ...
        numeros: [marcador, ...presiones.map((p) => p.score)],
        jugados: jugados,
      );
    }
  ...
  static String? quienPuedePedirCarry(
      Round round, String p1Id, String p2Id, BetModuleInstance mod) {
    final cfg = mod.nassau;
    if (!cfg.carryEnabled || cfg.soloElMatch) return null;
  ...
  static List<LedgerEntry> _nassauPair(Round round, String p1Id, String p2Id, BetModuleInstance mod) {
    ...
    if (cfg.aperturaB9For(p1Id, p2Id) && !cfg.soloElMatch && !seg.singleNine) {
  ...
  static void _addNassauSegment(
    ...
    // ── Una apuesta de \$0 no es una apuesta ────────────────────────────────
    //
    // Con «solo el match» —los dos nueves a cero— el F9 y el B9 producían un
    // asiento de cero pesos cada uno: en el desglose salían dos líneas que no
    // pagaban nada, y una línea que no paga se lee como algo que salió mal.
    if (value == 0) return;
    if (margin > 0) {
  ...
  static List<LedgerEntry> _nassauPressPair(
    ...
    void liquidateSegment({
      ...
      void addEntry(int score, double val, String label) {
        if (val == 0) return;   // ver _addNassauSegment
        if (score > 0) {
    ...
    if (seg.singleNine) {
      ...
      if (cfg.aperturaB9For(p1Id, p2Id) && !cfg.soloElMatch) {
        liquidateSegment(
          holes: seg.secondNine,
  ...
  static List<LedgerEntry> _nassauTeam(Round round, BetModuleInstance mod) {
    ...
    void addSegment(int margin, double value, String label) {
      if (margin == 0 || value == 0) return;   // ver _addNassauSegment
  ...
  static NassauPressLiveStatus nassauPressLiveStatus(
    ...
    // ── La guarda de `pressEnabled`, DENTRO ─────────────────────────────────
    //
    // Este método detectaba presiones mirara o no la configuración, y confiaba
    // en que cada llamador preguntara `mod.pressEnabled` antes. Los cuatro que
    // había lo hacían; el quinto —la línea «5 3 1»— tuvo que aprenderlo, y el
    // sexto habría enseñado a una partida sin presiones apuestas que nadie
    // abrió.
    //
    // Una guarda que hay que repetir en cada llamador no es una guarda: es una
    // costumbre. Ahora está donde no se puede olvidar.

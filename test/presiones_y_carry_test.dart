...
void main() {
  group('1 · el carry solo multiplica si el F9 empató', () {
    test('con el F9 EMPATADO el carry dobla el B9 y el total', () {
      ...
      final cfg = const NassauConfig(
          ...
          carryApplied: true,
          carryFactor: 2);
      final r = _r(ganaA: const [10, 11, 12], cfg: cfg);
      final b9 = _asientos(r).firstWhere((e) => e.reason.contains('Back 9'));
      expect(b9.amount, 100, reason: '50 doblado');
    });

    test('con el F9 GANADO no multiplica, aunque esté activado', () {
      // Es lo que el motor ya hacía bien; se fija para que no se pierda.
      final cfg = const NassauConfig(
          frontValue: 50,
          backValue: 50,
          totalValue: 100,
          ...
          carryApplied: true,
          carryFactor: 2);
      final r = _r(ganaA: const [1, 2, 3, 10, 11, 12], cfg: cfg);
      final b9 = _asientos(r).firstWhere((e) => e.reason.contains('Back 9'));
      expect(b9.amount, 50, reason: 'el F9 tuvo dueño: no hay nada que llevar');
    });

    test('2 · el multiplicador sigue siendo configurable', () {
      final cfg = const NassauConfig(
          frontValue: 50,
          backValue: 50,
          totalValue: 100,
      ...
      final r = _r(ganaA: const [10, 11, 12], cfg: cfg);
      final b9 = _asientos(r).firstWhere((e) => e.reason.contains('Back 9'));
      expect(b9.amount, 150, reason: '×3 es la variante del grupo');
    });
  });
